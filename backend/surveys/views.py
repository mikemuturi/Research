...
class SubmissionViewSet(viewsets.ModelViewSet):
    ...
    def destroy(self, request, *args, **kwargs):
        """Delete a submission (only for authenticated users)"""
        try:
    ...
    def update_comments(self, request, pk=None):
        """Update dimension comments for a submission"""
        submission = self.get_object()
        dimension_comments_data = request.data.get('dimension_comments', {})

        submission.dimension_comments.all().delete()
    ...
    def comments(self, request, pk=None):
        ...
        submission = self.get_object()
        comments = submission.dimension_comments.all()
        serializer = DimensionCommentSerializer(comments, many=True)
        return Response(serializer.data)

    @action(detail=False, methods=['get'])
    def comments_analysis(self, request):
        """Get qualitative analysis of comments"""
        queryset = self.get_queryset()
        ...
        analysis = {
            ...
            'submissions_with_comments': queryset.filter(
                dimension_comments__isnull=False
            ).distinct().count(),
        }

        for dimension in ['technical', 'economic', 'socio_cultural',
                          'environmental', 'policy_regulatory', 'strategic']:
            dim_comments = comments.filter(dimension=dimension)
            if dim_comments.exists():
                comment_texts = [c.comment for c in dim_comments]
                avg_length = sum(len(c) for c in comment_texts) / len(comment_texts)

                analysis['by_dimension'][dimension] = {
                    'count': dim_comments.count(),
                    'average_length': round(avg_length, 2),
                    'recent_comments': list(
                        dim_comments.order_by('-created_at')[:5]
                        .values('comment', 'created_at', 'submission__county')
                    )
                }

        if comments.exists():
            all_lengths = [len(c.comment) for c in comments]
            analysis['average_comment_length'] = round(
                sum(all_lengths) / len(all_lengths), 2
            )
