...
import { Download, Share2, BarChart3, FileText, ArrowLeft } from 'lucide-react';
...
const ResultsPage: React.FC = () => {
  ...
  const loadSubmission = async () => {
    ...
    try {
      setLoading(true);
      const response = await surveyAPI.getSubmission(id, false);
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      const response = await surveyAPI.exportPDF(id);
      const blob = new Blob([response.data], { type: 'application/pdf' });
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `rafsia_report_${id}.pdf`;
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mt-12 py-8">
          ...
          <Link to="/">
            <Button size="lg">Share the Assessment Tool</Button>
          </Link>
        </div>
      </div>
    </div>
